...
- `Ctrl+Shift+C` or `/copy-snippet`: opens the same picker showing the snippets and a preview. The picker supports navigation, paging, selection, and cancellation. Press `/` to enter opt-in search, type a query, then press `Esc` to leave search and restore the normal list. Search filters only snippet number, language, and the bounded one-line preview.
